...
class debug {
    ...
    */
    ...
    public function write() {
        ...
        if ( $this->_url ) {
            $str .= 'URL    : ' . $_SERVER[ 'REQUEST_SCHEME' ] . '://' . ( isset( $_SERVER[ 'HTTP_HOST_ORG' ] ) && $_SERVER[ 'HTTP_HOST_ORG' ] ? $_SERVER[ 'HTTP_HOST_ORG' ] :$_SERVER[ 'HTTP_HOST' ] ) . ( ( $_SERVER[ 'REQUEST_SCHEME' ] == 'http' && $_SERVER[ 'SERVER_PORT' ] == '80' ) || ( $_SERVER[ 'REQUEST_SCHEME' ] == 'https' && $_SERVER[ 'SERVER_PORT' ] == '443' ) ? '' : ':' . $_SERVER[ 'SERVER_PORT' ] ) . $_SERVER[ 'REQUEST_URI' ] . PHP_EOL;
        }
        if ( $this->_get && $_GET ) {
            $t = [];
            foreach ( $_GET as $k => $v ) {
